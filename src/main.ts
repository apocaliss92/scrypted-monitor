  ...
  implements HttpRequestHandler
{
    ...
    try {
      ...
      if (webhook === healthcheck) {
        ...
        if (!lastRun || msFromLastRun > 1000 * 30) {
          ...
          const detectionResult = await executeWorkerDetections({
            workerName,
            detectionsToRun,
          });
          ...
          response.send(
            `Need to wait at least 30 seconds between healthchecks, ${30 - msFromLastRun / 1000} seconds remaining`,
            {
              code: 500,
            },
          );
  ...
  async start(shouldLog: boolean) {
    ...
    if (sdk.clusterManager?.getClusterMode?.()) {
      ...
      for (const worker of workers) {
        const { healthcheckUrl } = await getWebHookUrls({
  ...
  getTask(taskName: string): Task {
    ...
    return {
      ...
      rebootOnErrors: JSON.parse(
        this.storage.getItem(taskRebootKey) ?? "false",
      ),
      skipNotify: JSON.parse(this.storage.getItem(taskSkipNotify) ?? "false"),
      runSystemDiagnostic: JSON.parse(
        this.storage.getItem(taskSystemDiagnostic) ?? "true",
      ),
      beta: JSON.parse(this.storage.getItem(taskBetaKey) ?? "false"),
      plugins: JSON.parse(
        (this.storage.getItem(taskPluginsKey as any) as string) ?? "[]",
      ),
      pluginEventsLevel:
        (this.storage.getItem(taskPluginEventsLevelKey) as any) || "warning",
      devices: JSON.parse(
        (this.storage.getItem(taskDevicesKey as any) as string) ?? "[]",
      ),
      additionalNotifiers: JSON.parse(
        (this.storage.getItem(taskAdditionalNotifiers as any) as string) ??
          "[]",
      ),
      enabled: JSON.parse(this.storage.getItem(taskEnabledKey) ?? "true"),
      checkAllPlugins: JSON.parse(
        this.storage.getItem(taskCheckAllPluginsVersion) ?? "true",
      ),
      maxStats: JSON.parse(this.storage.getItem(taskMaxStatsKey) ?? "5"),
      rpcThreshold: this.storage.getItem(taskRpcObjects)
        ? JSON.parse(this.storage.getItem(taskRpcObjects))
        : "",
      entitiesToAlwaysReport: JSON.parse(
        (this.storage.getItem(taskEntitiesToAlwaysReport as any) as string) ??
          "[]",
      ),
      entitiesToExclude: JSON.parse(
        (this.storage.getItem(taskEntitiesToExclude as any) as string) ?? "[]",
      ),
      batteryThreshold: JSON.parse(
        this.storage.getItem(taskBatteryThreshold) ?? "30",
      ),
      unavailableTime: JSON.parse(
        this.storage.getItem(tasksUnavailableTime) ?? "24",
      ),
      calendarDaysInFuture: JSON.parse(
        this.storage.getItem(taskCalendarDaysInFuture) ?? "14",
      ),
  ...
  async executeTask(task: Task) {
    ...
    if (type === TaskType.Diagnostics) {
      for (const deviceId of devices) {
        const device = sdk.systemManager.getDeviceById(
          deviceId,
        ) as unknown as ScryptedDeviceBase & Reboot;
        logger.log(`Starting ${type} for ${device.name}`);
        const result = await runValidate(
          this.diagnosticsPlugin,
          logger,
          deviceId,
        );
        ...
        if (
          rebootOnErrors &&
          result.errorSteps.length > 0 &&
          device.interfaces.includes(ScryptedInterface.Reboot)
        ) {
          message += ` | Restarting |`;
      ...
      if (threshold > 0) {
        logger.log(
          `Skipping ${type} execution because rpcThreshold is set (${threshold}). This task runs via 10s interval.`,
        );
        forceStop = true;
      } else {
      ...
      const levelRank = (level: string | undefined) => {
        ...
        if (
          ...
          return 2;
        if (l === "w" || l.includes("warn")) return 1;
        return 0;
      };
      ...
      for (const pluginName of selectedPlugins) {
        ...
        const selected = sample.filter(
          (l) => levelRank(l.level) >= thresholdRank,
        );
        if (!selected.length) {
          continue;
        }

        const warnCount = selected.filter(
          (l) => levelRank(l.level) === 1,
        ).length;
        const errorCount = selected.filter(
          (l) => levelRank(l.level) === 2,
        ).length;
        const infoCount = selected.filter(
          (l) => levelRank(l.level) === 0,
        ).length;
      ...
      const pluginsToUpdate = isAllPlugins
        ? allPlugins.map((plugin) => plugin.id)
        : plugins;
      ...
      if (checkAllPlugins && !isAllPlugins) {
        const otherPlugins = allPlugins.filter(
          (plugin) => !plugins.includes(plugin.id),
        );
        logger.log(
          `Checking other plugins: ${otherPlugins.map((plugin) => plugin.name)}`,
        );

        for (const plugin of otherPlugins) {
          const { manufacturer, version } = plugin.info;
          const { newVersion, isBeta } = await pluginHasUpdate(
            logger,
            manufacturer,
            version,
            beta,
      ...
      if (stats.benchmark) {
        message += `[Benchmark]\n`;
        stats.benchmark.detectorsStats.forEach(
          (item) =>
            (message += `${item.name}: ${item.detections} detections in ${item.time} seconds (${item.detectionRate} dps)\n`),
        );
      ...
      if (stats.rpcObjects) {
        message += `[RPC Objects]\n`;
        stats.rpcObjects.forEach(
          (item) => (message += `${item.name}: ${item.count}\n`),
        );
        message += `${divider}\n`;
      }
      ...
      if (stats.cluster) {
        message += `[Workers]\n`;
        stats.cluster.workers.forEach(
          (item) => (message += `${item.name}: ${item.count}\n`),
        );
        message += `${divider}\n`;

        message += `[Devices]\n`;
        stats.cluster.devices.forEach(
          (item) => (message += `${item.name}: ${item.count}\n`),
        );
      }
      ...
      statuses.data.forEach((entity) => {
        if (
          ...
          (entitiesToAlwaysReport?.length
            ? entitiesToAlwaysReport.some((regex) =>
                new RegExp(regex).test(entity.entity_id),
              )
            : true) &&
          (entitiesToExclude?.length
            ? entitiesToExclude.every(
                (regex) => !new RegExp(regex).test(entity.entity_id),
              )
            : true)
        ) {
          const lastUpdate =
            entity.attributes.last_seen ?? entity.last_reported;
          let messageToAdd = `${entity?.attributes?.friendly_name}`;
          if (lastUpdate) {
            const timeString = moment(lastUpdate)
              .locale(datesLocale)
              .calendar();
      ...
      const endDate = moment()
        ...
        .replace("T", " ")
        .split(".")[0];
      const endDateFuture = moment()
        .add(calendarDaysInFuture, "days")
        .endOf("day")
        .toISOString()
        .replace("T", " ")
        .split(".")[0];
      ...
      const events =
        eventsResponse.data.service_response[calendarEntity]?.events;
      ...
      const eventsFuture =
        eventsResponseFuture.data.service_response[calendarEntity]?.events;
      logger.log(
        `Events found in the next ${calendarDaysInFuture} days: ${eventsFuture}`,
      );
    ...
    if (!skipNotify && !forceStop) {
      ...
      if (additionalNotifiers?.length) {
        for (const notifierId of additionalNotifiers) {
          notifiers.push(
            sdk.systemManager.getDeviceById(
              notifierId,
            ) as unknown as ScryptedDeviceBase & Notifier,
          );
        }
      } else if (notifier) {
      ...
      for (const notifierDevice of notifiers) {
        logger.log(
          `Sending notification to ${notifierDevice.name}: ${JSON.stringify({ title, message })}`,
        );
  ...
  private async migrateCamera() {
    ...
    const source = sdk.systemManager.getDeviceById<Settings>(
      cameraMigrationSource.id,
    );
    const target = sdk.systemManager.getDeviceById<Settings>(
      cameraMigrationTarget.id,
    );
    ...
    logger.log(
      `Camera migration: ${source.name} (${cameraMigrationSource}) -> ${target.name} (${cameraMigrationTarget}): keys: ${cameraMigrationKeys.join(", ")}`,
    );
    ...
    if (cameraMigrationKeys.includes(MigrationKeys.NVRRecordings)) {
      const nvrDevice =
        sdk.systemManager.getDeviceByName<Settings>("Scrypted NVR");
      if (!nvrDevice) {
        logger.log(
          "Camera migration: Scrypted NVR device not found, skipping NVR recordings migration",
        );
      } else {
        const nvrSettings = await nvrDevice.getSettings();
        const recordingSettingsKey = nvrSettings.find(
          (s) => s.key === "recordingsPath",
        )?.value;

        if (!recordingSettingsKey || typeof recordingSettingsKey !== "string") {
          logger.log(
            "Camera migration: recordingsPath is not configured, cannot read NVR folders",
          );
        } else {
          try {
            ...
            logger.log(
              "Camera migration: error reading recordingsPath",
    ...
    if (cameraMigrationKeys.includes(MigrationKeys.Extensions)) {
      ...
      target.setMixins(uniq([...target.mixins, ...srcMixins]));

      source.setMixins([]);
    }

    logger.log(
      `Camera migration: completed (settings copied: ${migratedCount})`,
    );
  }
  ...
  async startTaskCron(task: Task) {
    const logger = this.getLogger();
    try {
      const { cronScheduler, name, type, rpcThreshold } = task;
  ...
  private updatePluginEventsMonitoringInterval(taskEntities: Task[]) {
    ...
    const shouldRun = enabledTaskEntities.some(
      (task) =>
    ...
    this.pluginEventsMonitorInterval = setInterval(() => {
      this.pluginEventsMonitorTick().catch((e) =>
        logger.log("PluginEventsMonitoring tick error", e),
      );
    }, 10_000);
  ...
  private async pluginEventsMonitorTick() {
    ...
    try {
      const taskEntities = await this.getTasks();
      const pluginEventTasks = taskEntities.filter(
        (task) =>
          task.type === TaskType.PluginEventsMonitoring &&
          !!task.plugins?.length &&
          task.enabled,
      ...
      const restartRpcTasks = taskEntities.filter(
        (task) =>
          task.type === TaskType.RestartPlugins &&
          Number(task.rpcThreshold || 0) > 0 &&
          !!task.plugins?.length &&
      ...
      const signature = (
        entry: Awaited<ReturnType<typeof fetchPluginEvents>>[number],
      ) => {
      ...
      for (const task of pluginEventTasks) {
        ...
        for (const pluginName of plugins) {
          ...
          try {
            ...
            const err = e as any;
            lines.push(
              `[${pluginName}]: error fetching logs: ${err?.message || err}`,
            );
            continue;
          }
          ...
          const lastSeen =
            this.pluginEventsLastSeenByTaskPlugin.get(taskPluginKey);
          ...
          const unreadMatchingThreshold = unread.filter(
            (l) => levelRank(l.level) >= thresholdRank,
          );
          ...
          lines.push(
            `[${pluginName}] (${task.pluginEventsLevel || "warning"})`,
          );
        ...
        for (const notifierDevice of notifiers) {
          for (let i = 0; i < chunks.length; i++) {
            const title =
              chunks.length === 1
                ? `${task.name}`
                : `${task.name} (${i + 1}/${chunks.length})`;
            logger.log(
              `PluginEventsMonitoring notify ${notifierDevice.name}: ${JSON.stringify({ title })}`,
            );
            await notifierDevice.sendNotification(title, {
              body: chunks[i],
            });
        ...
        for (const task of restartRpcTasks) {
          ...
          for (const pluginId of plugins) {
            ...
            if (rpcObjects > threshold) {
              lines.push(
                `[${manufacturer}] rpcObjects=${rpcObjects} > ${threshold} -> restarting`,
              );
            ...
            for (let i = 0; i < chunks.length; i++) {
              const title =
                chunks.length === 1
                  ...
                  : `${task.name} (${i + 1}/${chunks.length})`;
              logger.log(
                `RestartPlugins (rpcThreshold) notify ${notifierDevice.name}: ${JSON.stringify({ title })}`,
              );
              await notifierDevice.sendNotification(title, {
                body: chunks[i],
              });
  ...
  async getSettings() {
    ...
    (tasks as string[]).forEach((task) => {
      ...
      if (
        (type !== TaskType.RestartPlugins || !rpcThreshold) &&
        type !== TaskType.PluginEventsMonitoring
      ) {
        settings.push({
          key: taskCronKey,
          title: "Cron",
